DATAS SEGMENT
    str_out	DB	'Input a string(<10):', 0AH, 0DH, '$'   ;0AH:ASCII码换行,0DH:ASCII码回车, '$'字符串结束标志
    str_in  DB  11, ?, 10 DUP(?)                      ;串中第1个元素规定允许最多输入的字符数(含回车),此处最多可输入10个字符
                                                        ;串中第2个元素保留,保存输入字符的个数
                                                        ;从第3个元素起为输入字符的缓冲区    
    str_buf DB  13 DUP(?)                              ;长度为100字节的字符串缓冲区
    var1    DB 1
    var2    DB 2
    result  DB ?
